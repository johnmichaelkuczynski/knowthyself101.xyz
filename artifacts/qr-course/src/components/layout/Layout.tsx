...
import { LayoutDashboard, BookOpen, PenTool, BarChart3, Activity, RotateCcw, Sparkles } from "lucide-react";
      ...
      <div className="p-6 border-b border-border">
        <Link href="/">
          <div className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="font-serif font-semibold text-lg tracking-tight">Know Thyself</span>
          </div>
        </Link>
      </div>
...
function TopBar() {
  ...
  async function handleExpandLectures() {
    if (
      !confirm(
        "Generate Medium and Long versions of every lecture? This runs the writer over all 29 lectures twice (medium, then long). Takes a few minutes.",
      )
    )
      return;
    setExpanding(true);
